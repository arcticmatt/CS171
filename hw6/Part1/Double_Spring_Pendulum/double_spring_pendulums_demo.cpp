...
void update_pendulums()
{
    ...
    /*
     * To get the continuous Laplacian, we just add the kinetic and potential
     * energies for each pendulum.
     *
     * This is pretty simple. There is only one catch. When calculating the
     * potential energy of the second (attached) pendulum, we use
     * sqrt((x2 - x1)^2 + (y2 - y1)^2) for the stretch distance. That is,
     * we must take into account the fact that its position is affected by
     * the first pendulum, and that it is not in fact stretched as far as its
     * position suggests.
     *
     * After taking this into account, everything else is fairly straighforward.
     * We just calculate all 8 momentum variables, and from those, do the updates.
     */
    ...
    float sqrt_val1 = sqrt(pow(m1.x, 2) + pow(m1.y, 2));
    float sqrt_val2 = sqrt(pow(m2.x - m1.x, 2) + pow(m2.y - m1.y, 2));
    float sqrt_frac1 = (sqrt_val1 - m1.rl) / sqrt_val1;
    float sqrt_frac2 = (sqrt_val2 - m2.rl) / sqrt_val2;

    // Update momentum for mass 1
    m1.px += dt * ( -(m1.k * m1.x * sqrt_frac1) + (m2.k * (m2.x - m1.x) * sqrt_frac2) );
    m1.py += dt * ( -(m1.k * m1.y * sqrt_frac1) + (m2.k * (m2.y - m1.y) * sqrt_frac2)
                        + m1.m * g );

    // Update momentum for mass 2
    m2.px += dt * -(m2.k * (m2.x - m1.x) * sqrt_frac2);
    m2.py += dt * ( -(m2.k * (m2.y - m1.y) * sqrt_frac2) + m2.m * g );

    /*** Use updated momentum variables to update position variables ***/
    m1.x += (dt * m1.px) / m1.m;
    m1.y += (dt * m1.py) / m1.m;
    m2.x += (dt * m2.px) / m2.m;
    m2.y += (dt * m2.py) / m2.m;
